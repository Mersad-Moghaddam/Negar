...
function GoalStatusPill({ status }: { status: GoalPeriodOverview['status'] }) {
  const { t } = useI18n()
  return (
    <span className="rounded-full bg-muted px-2 py-1 text-xs text-mutedForeground">
      {t(`dashboard.goalStatus.${status}`)}
    </span>
  )
}
    ...
    <div className="space-y-2 rounded-xl border border-border bg-surface p-3">
      <div className="flex items-center justify-between">
        <p className="font-medium">
          {item.period === 'weekly' ? t('dashboard.periodWeekly') : t('dashboard.periodMonthly')}
        </p>
        <GoalStatusPill status={item.status} />
      </div>
      <p className="text-xs text-mutedForeground">
        {t('dashboard.goalSummary', {
          pagesRead: numberFormatter.format(item.pagesRead),
          pagesGoal: numberFormatter.format(pagesTarget),
          booksRead: numberFormatter.format(item.booksRead),
          booksGoal: numberFormatter.format(booksTarget)
        })}
      </p>
      <Progress value={Math.min(100, item.percent)} />
      <p className="text-xs text-mutedForeground">
        {t('dashboard.goalPercent', { percent: numberFormatter.format(item.percent) })}
      </p>
    </div>
  )
}
...
export function ReadingGoalsCard({
  ...
  const [draft, setDraft] = useState<{ weekly: GoalDraft; monthly: GoalDraft }>({
    weekly: {},
    monthly: {}
  })
      ...
      {goals?.suggestions.length ? (
        <div className="space-y-2 rounded-xl border border-border bg-surface p-3">
          ...
          {goals.suggestions.map((s) => (
            <div
              key={s.period}
              className="flex items-center justify-between gap-3 text-xs text-mutedForeground"
            >
              <div>
                <p className="font-medium text-foreground">
                  {s.period === 'weekly'
                    ? t('dashboard.periodWeekly')
                    : t('dashboard.periodMonthly')}
                </p>
                <p>
                  {t('dashboard.goalSummary', {
                    pagesRead: numberFormatter.format(0),
                    pagesGoal: numberFormatter.format(s.targetPages ?? 0),
                    booksRead: numberFormatter.format(0),
                    booksGoal: numberFormatter.format(s.targetBooks ?? 0)
                  })}
                </p>
                <p>
                  {s.reasonKey
                    ? t(`dashboard.goalSuggestionReasons.${s.reasonKey}`)
                    : t('dashboard.goalSuggestionReasons.fallback')}
                </p>
              </div>
              <Button size="sm" variant="secondary" onClick={() => applySuggestion(s.period)}>
      ...
      {editing ? (
        <div className="space-y-3 rounded-xl border border-border bg-surface p-3">
          ...
          {(['weekly', 'monthly'] as const).map((period) => (
            <div key={period} className="grid grid-cols-2 gap-2">
              <Input
                placeholder={t('dashboard.pagesGoal')}
                type="number"
                min={0}
                value={draft[period].pages ?? ''}
                onChange={(e) =>
                  setDraft((prev) => ({
                    ...prev,
                    [period]: { ...prev[period], pages: e.target.value }
                  }))
                }
              />
              <Input
                placeholder={t('dashboard.booksGoal')}
                type="number"
                min={0}
                value={draft[period].books ?? ''}
                onChange={(e) =>
                  setDraft((prev) => ({
                    ...prev,
                    [period]: { ...prev[period], books: e.target.value }
                  }))
                }
              />
            </div>
          ...
          <div className="flex gap-2">
            <Button
              ...
              onClick={async () => {
                await onSave({
                  weekly: {
                    pages: toNumber(draft.weekly.pages),
                    books: toNumber(draft.weekly.books)
                  },
                  monthly: {
                    pages: toNumber(draft.monthly.pages),
                    books: toNumber(draft.monthly.books)
                  }
                })
                setEditing(false)
              }}
        ...
        <div className="flex gap-2">
          ...
          <Button
            size="sm"
            variant="ghost"
            disabled={isSaving || !goals?.suggestions.length}
            onClick={async () => onSave({ applySuggestion: true })}
          >
            {t('dashboard.useSuggestions')}
          </Button>
        </div>
      )}
    </div>
