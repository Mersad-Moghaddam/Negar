    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      <SectionCard>
        <SectionHeader
          ...
          description={t('wishlist.addDescription')}
        ...
        <form
          onSubmit={itemForm.handleSubmit(async (values) =>
            addItem.mutateAsync({
              ...values,
              expectedPrice: Number.isNaN(values.expectedPrice)
                ? null
                : (values.expectedPrice ?? null)
            })
          )}
        ...
        >
          ...
          <Button
            type="submit"
            className="w-full md:col-span-2 md:w-fit"
            disabled={addItem.isPending}
          >
            {t('wishlist.addAction')}
          </Button>
        </form>
      </SectionCard>
      <QueryState
        ...
        emptyDescription={t('wishlist.emptyDescription')}
      >
        <div className="grid gap-3 md:grid-cols-2">
          {query.data?.map((item) => (
            <SectionCard key={item.id}>
                ...
                <div>
                  ...
                  {item.expectedPrice != null ? (
                    <p className="text-xs text-mutedForeground">
                      {t('wishlist.expectedPrice')}: {numberFormatter.format(item.expectedPrice)}
                    </p>
                  ) : null}
                </div>
              ...
              {confirmingDeleteId === item.id ? (
                <p className="text-xs text-mutedForeground">{t('wishlist.deleteConfirm')}</p>
              ) : null}
              <Separator />
              <WishlistLinkForm
                itemId={item.id}
                ...
                <div className="space-y-2">
                  {item.purchaseLinks.map((link) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center justify-between gap-3 rounded-lg border border-border bg-surface px-3 py-2 text-xs text-mutedForeground hover:bg-secondary"
                    >
                      <span className="truncate">{link.label || link.alias}</span>
